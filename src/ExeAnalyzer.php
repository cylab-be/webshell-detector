...
class ExeAnalyzer implements Analyzer
{
    ...
    public function analyze($string)
    {
        ...
        if ($anonymous < self::MIN_ANONYMOUS) {
            ...
            $anonymous = ($anonymous - self::MIN_ANONYMOUS) /
                    (self::MAX_ANONYMOUS - self::MIN_ANONYMOUS);
        }
        ...
        if ($varfunc < self::MIN_VARFUNC) {
            ...
            $varfunc = ($varfunc - self::MIN_VARFUNC) /
                    (self::MAX_VARFUNC - self::MIN_VARFUNC);
        }
    ...
    private function _searchExecCmdFunctions($tokens)
    {
        $count = 0;
        $funcs = array(
            "exec", "assert", "passthru", "popen", "proc_open", "pcntl_exec",
            "shell_exec", "system", "python_eval");
