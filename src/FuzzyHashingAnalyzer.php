...
/**
 * Class FuzzyHashingAnalyzer. Computes a hash using a Context-Triggered Piecewise
 * Hashing algorithm.
 * This kind a algorithm can be used to detect similar files. Whereas hash functions
 * like MD5 or SHA-256 will produce different strings even if inputs are close,
 * fuzzy hashing will produce similar outputs.
...
class FuzzyHashingAnalyzer implements Analyzer
{
    ...
    public function analyze($filecontent)
    {
        ...
        if ($filecontent != null && is_string($filecontent)
            ...
            $filecontent = Util::removeAllWhiteSpaces(
                Util::removeComments($filecontent)
            );
